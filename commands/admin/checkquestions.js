					...
					if (results.length == 0) {
						...
						try {
							...
							if (confirmation.customId === 'accepter') {
								connection.query('UPDATE questions SET verified = ?, valid = ? WHERE user_id = ? AND id = ?', [1, 1, interaction.user.id, results[0].id], async function (error, rezz, fields) {
									if (error) throw error;
									console.log(rezz);
									connection.query('UPDATE users SET points = points + 0.2 WHERE user_id = ?', [interaction.user.id], async function (error, re, fields) {
										if (error) throw error;
										console.log(re);
									});
									connection.query('INSERT INTO points (user_id, points, reason) VALUES (?, ?, ?)', [interaction.user.id, 0.2, "Question acceptée"], async function (error, ea, fields) {
										if (error) throw error;
										console.log(ea);
									});
									connection.query('SELECT * FROM important WHERE name = "channel"', async function (error, hehe, fields) {
										...
										console.log(hehe);
										if (hehe.length > 0) {
											await interaction.client.channels.cache.get(hehe[0].value).send('<@' + interaction.user.id + '>, votre question a été validée. Vous avez gagné 0.2 points !');
								...
								connection.query('SELECT * FROM questions WHERE verified = 0 AND id =' + results[0].id, async function (error, results, fields) {
									...
									if (results.length == 0) {
										...
										connection.query('UPDATE questions SET verified = ?, valid = ? WHERE user_id = ? AND id = ?', [true, 0, interaction.user.id, results[0].id], async function (error, era, fields) {
											if (error) throw error;
											console.log(era);
											...
											connection.query('SELECT * FROM important WHERE name = "channel"', async function (error, result, fields) {
												...
												console.log(result);
												if (result.length > 0) {
													await interaction.client.channels.cache.get(result[0].value).send('<@' + interaction.user.id + '>, votre question n\'a pas été validée. Vous n\'avez donc pas gagné de point.  ');
													await interaction.reply('La réponse a été envoyée dans le salon correspondant.')
												} else {
